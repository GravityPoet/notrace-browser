...
| 2026-08-25 | v0.1.0 | GitHub Actions run `32751861884`, `./packaging/test-picker-native-e2e.sh` | `等待超时：账号目录复制动作`; `navigator.clipboard.writeText` never settled in the signed WebView | The native macOS WebView exposed Clipboard API but did not resolve the promise for the real click, while the UI awaited it indefinitely. | Add a 750 ms timeout and fall back to the existing DOM copy path; rebuilt installed-app E2E then passed all six checks. | Native copy actions must have a bounded Clipboard API wait and a recoverable fallback; never let a platform clipboard promise block the UI indefinitely. |
